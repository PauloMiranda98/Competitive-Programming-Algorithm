...
typedef tuple<long int, long int, long int, long int> tiiii;

priority_queue< pii, vector<pii>, greater<pii> > fila;

long int menor[MAXN][PERIODO];
vector<tiiii> adj[MAXN];
	...
	while(!fila.empty()){
		...
		for(tiiii aux: adj[v]){
			...
			if( (t+d)%(a+f) < a){ //chegou e o sinal estava aberto
				if(t+d < menor[u][(t+d)%PERIODO]){
					menor[u][(t+d)%PERIODO] = t+d;
					fila.push(pii(t+d, u));
				}
			}else{ //sinal fechou
				if((t+d+(a+f - ((t+d)%(a+f)) )) < menor[u][(t+d)%PERIODO]){
					menor[u][(t+d)%PERIODO] = (t+d+(a+f - ((t+d)%(a+f)) ));
