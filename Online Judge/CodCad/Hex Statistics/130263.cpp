...
ll pd_min(int mask){
	...
	if(memo_min[mask] != -1LL)
		return memo_min[mask];
		
	ll res = 0x3f3f3f3f3f3f3f3fLL;
		
	for(int i=0; i<16; i++)
		if( mask&(1<<i) )
			res = min(res, pd_min(mask&(~(1<<i))));
		
	return memo_min[mask] = res + valor(mask);
}
...
ll pd_max(int mask){
	...
	if(memo_max[mask] != -1LL)
		return memo_max[mask];
		
	ll res = -0x3f3f3f3f3f3f3f3fLL;
		
	for(int i=0; i<16; i++)
		if( mask&(1<<i) )				
			res = max(res, pd_max(mask&(~(1<<i))) );
		
	return memo_max[mask] = res + valor(mask);
}
...
ll pd(int mask){
	...
	return memo[mask] = (res + fat(mask) )%MOD;
}
